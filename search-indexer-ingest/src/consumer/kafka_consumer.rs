...
impl KafkaConsumer {
                        ...
                        Some(Ok(msg)) => {
                            ...
                            match self.parse_message(&msg) {
                                ...
                                Ok(None) => {
                                    ...
                                    // Still commit offset immediately (no processing needed)
    ...
    fn parse_message(
        ...
        let events = if topic == KNOWLEDGE_EDITS_TOPIC {
            match self.parse_edit_message(payload, offset, msg) {
                Ok(events) => events,
                Err(e) => {
                    error!(
                        topic = %topic,
                ...
                match payload {
                    ...
                    Payload::DeleteRelation(relation_id) => {
                        ...
                        if let Ok(id_bytes) = transform_id_bytes(relation_id.clone()) {
                            ...
                            debug!(
                                relation_id_bytes = ?relation_id,
                                space_id = %space_id,
                                edit_name = %edit.name,
                                cursor = %cursor,
                                "Skipped relation deletion (invalid relation ID)"
                            );
                        }
                    }
                    _ => {
    ...
    fn process_update_entity(
        ...
        // Create an event even if name is None (names are now optional)
